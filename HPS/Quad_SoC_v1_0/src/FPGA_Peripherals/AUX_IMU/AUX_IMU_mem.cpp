...
//											INCLUDES											//
//==============================================================================================//
#include "AUX_IMU_mem.hpp"	// header
#include "AUX_IMU.h"	// header
...
AUX_IMU_mem::AUX_IMU_mem(uint32_t * aux_imu_memory_virtual_address, int aux_imu_memory_length_dword):API_mem(aux_imu_memory_virtual_address){
	AUX_IMU_mem::memory_length = aux_imu_memory_length_dword;
	init_aux_imu(&(AUX_IMU_mem::memory_map), 0x00000000);
	std::cout << "memory_map reset bit" << memory_map.control.reset_n << std::endl;
};

AUX_IMU_mem::~AUX_IMU_mem(){};



// extra
int AUX_IMU_mem::hard_reset(){
...
int AUX_IMU_mem::print(int start, int end){
	if( (start >= 0) or (start < memory_length) ){
		if( (end >= start) or (end < memory_length) ){
			...
			std::cout << "AUX_IMU_mem::print(int , int) > 2nd argument wrong, min value(1st argument)/argument/max value : " << start << "/" << end << "/" << memory_length << std::endl;
		}
	}else{
		std::cout << "AUX_IMU_mem::print(int , int) > 1st argument wrong, min value/argument/max value = " << 0 << "/" << start << "/" << memory_length << std::endl;
	}
	return _ERROR_;
};
...
// control register
int AUX_IMU_mem::set_control_reg(uint32_t value){
	uint32_t p_value = value;
	API_mem::write_dword(&p_value, memory_map.control.offset, 1);
	return _SUCCESS_;
};
int AUX_IMU_mem::clear_control_reg(){
	uint32_t p_value = 0x00000000;
	API_mem::write_dword(&p_value, memory_map.control.offset, 1);
	return _SUCCESS_;
};
...
// hsp2aux
int AUX_IMU_mem::get_aux2hps_length(){
	// returns max size of aux2hps memory in dword
	return memory_map.aux2hps.length;
};

int AUX_IMU_mem::load_aux2hps_data(int32_t * data, int length){
...
// routine control reg
int AUX_IMU_mem::get_routine_count(){
	return memory_map.routine.count;
};
int AUX_IMU_mem::set_routine_control_reg(uint32_t value){
	uint32_t p_value = value;
	API_mem::write_dword(&p_value, memory_map.routine.control.offset, 1);
	return _SUCCESS_;
};
int AUX_IMU_mem::clear_routine_control_reg(){
	uint32_t p_value = 0x00000000;
	API_mem::write_dword(&p_value, memory_map.routine.control.offset, 1);
	return _SUCCESS_;
};
// bits
int AUX_IMU_mem::set_routine_control_enable(int which){
...
int AUX_IMU_mem::clear_routine_control_enable(int which){
...
// routine status reg
uint32_t AUX_IMU_mem::get_routine_status_reg(){
	uint32_t p_value;
...
// bits
bool AUX_IMU_mem::check_routine_status_flag_enable(int which){
	return API_mem::check_bit(memory_map.routine.status.offset, memory_map.routine.status.flag_enable[which]);
};
bool AUX_IMU_mem::check_routine_status_flag_enable(int which, uint32_t * error){
	if( (which >= 0) or (which < (int)memory_map.routine.count) ){
		return API_mem::check_bit(memory_map.routine.status.offset, memory_map.routine.status.flag_enable[which]);
		*error = 0x00000000;
	}
...
bool AUX_IMU_mem::check_routine_status_flag_pkt_rdy(int which){
	return API_mem::check_bit(memory_map.routine.status.offset, memory_map.routine.status.flag_pkt_rdy[which]);
};
bool AUX_IMU_mem::check_routine_status_flag_pkt_rdy(int which, uint32_t * error){
	if( (which >= 0) or (which < (int)memory_map.routine.count) ){
		return API_mem::check_bit(memory_map.routine.status.offset, memory_map.routine.status.flag_pkt_rdy[which]);
		*error = 0x00000000;
	}
...
bool AUX_IMU_mem::check_routine_status_flag_valid(int which){
	return API_mem::check_bit(memory_map.routine.status.offset, memory_map.routine.status.flag_valid[which]);
};
bool AUX_IMU_mem::check_routine_status_flag_valid(int which, uint32_t * error){
	if( (which >= 0) or (which < (int)memory_map.routine.count) ){
		return API_mem::check_bit(memory_map.routine.status.offset, memory_map.routine.status.flag_valid[which]);
		*error = 0x00000000;
	}
...
// clear
int AUX_IMU_mem::clear_routine_status_flag_enable(int which){
	API_mem::clear_bit(memory_map.routine.status.offset, memory_map.routine.status.flag_enable[which]);
	return _SUCCESS_;
}
int AUX_IMU_mem::clear_routine_status_flag_enable(int which, uint32_t * error){
	if( (which >= 0) or (which < (int)memory_map.routine.count) ){
...
int AUX_IMU_mem::clear_routine_status_flag_pkt_rdy(int which){
	API_mem::clear_bit(memory_map.routine.status.offset, memory_map.routine.status.flag_pkt_rdy[which]);
	return _SUCCESS_;
}
int AUX_IMU_mem::clear_routine_status_flag_pkt_rdy(int which, uint32_t * error){
	if( (which >= 0) or (which < (int)memory_map.routine.count) ){
...
int AUX_IMU_mem::clear_routine_status_flag_valid(int which){
	API_mem::clear_bit(memory_map.routine.status.offset, memory_map.routine.status.flag_valid[which]);
	return _SUCCESS_;
}
int AUX_IMU_mem::clear_routine_status_flag_valid(int which, uint32_t * error){
	if( (which >= 0) or (which < (int)memory_map.routine.count) ){
...
// check_clear
bool AUX_IMU_mem::check_clear_routine_status_flag_enable(int which){
	uint32_t register_value;
	// get register value
	API_mem::read_dword(&register_value, memory_map.routine.status.offset, 1);
	// check
	if( (register_value & memory_map.routine.status.flag_enable[which]) > 0){
		// modify
		register_value = register_value & (~memory_map.routine.status.flag_enable[which]);
		// write
		API_mem::write_dword(&register_value, memory_map.routine.status.offset, 1);
		return true;
	}
	// no modification required
	return false;
};
bool AUX_IMU_mem::check_clear_routine_status_flag_enable(int which, uint32_t * error){
	if( (which >= 0) or (which < (int)memory_map.routine.count) ){
		*error = 0x00000000;
		uint32_t register_value;
		// get register value
...
bool AUX_IMU_mem::check_clear_routine_status_flag_pkt_rdy(int which){
	uint32_t register_value;
	// get register value
	API_mem::read_dword(&register_value, memory_map.routine.status.offset, 1);
	// check
	if( (register_value & memory_map.routine.status.flag_pkt_rdy[which]) > 0){
		// modify
		register_value = register_value & (~memory_map.routine.status.flag_pkt_rdy[which]);
		// write
		API_mem::write_dword(&register_value, memory_map.routine.status.offset, 1);
		return true;
	}
...
bool AUX_IMU_mem::check_clear_routine_status_flag_pkt_rdy(int which, uint32_t * error){
	if( (which >= 0) or (which < (int)memory_map.routine.count) ){
		*error = 0x00000000;
		uint32_t register_value;
		// get register value
...
bool AUX_IMU_mem::check_clear_routine_status_flag_valid(int which){
	uint32_t register_value;
	// get register value
	API_mem::read_dword(&register_value, memory_map.routine.status.offset, 1);
	// check
	if( (register_value & memory_map.routine.status.flag_valid[which]) > 0){
		// modify
		register_value = register_value & (~memory_map.routine.status.flag_valid[which]);
		// write
		API_mem::write_dword(&register_value, memory_map.routine.status.offset, 1);
		return true;
	}
...
bool AUX_IMU_mem::check_clear_routine_status_flag_valid(int which, uint32_t * error){
	if( (which >= 0) or (which < (int)memory_map.routine.count) ){
		*error = 0x00000000;
		uint32_t register_value;
		// get register value
...
// routine packet
int AUX_IMU_mem::get_routine_packet_length(){
	// returns max size of packet memory in dword
	return memory_map.routine.packet.length;
};
int AUX_IMU_mem::get_routine_packet(int which, int32_t * packet, int length){
